...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof incrmpcorr2, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function throws an error if not provided a number as the mean value', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			incrmpcorr2( 3, 3.14, value );
		};
	}
});

tape( 'the function returns an accumulator function', function test( t ) {
	t.strictEqual( typeof incrmpcorr2( 3 ), 'function', 'returns expected value' );
	t.end();
});

tape( 'the function returns an accumulator function (known means)', function test( t ) {
	t.strictEqual( typeof incrmpcorr2( 3, 3.0, 3.14 ), 'function', 'returns expected value' );
	t.end();
});
...
tape( 'the accumulator function computes a moving squared sample Pearson product-moment correlation coefficient incrementally (known means)', function test( t ) {
	...
	for ( i = 0; i < N; i++ ) {
		...
		acc = incrmpcorr2( W, means[ 0 ], means[ 1 ] );
		for ( j = 0; j < M; j++ ) {
			actual = acc( d[j][0], d[j][1] );
			if ( j < W ) {
				arr = d.slice( 0, j+1 );
...
tape( 'if not provided an input value, the accumulator function returns the current squared sample correlation coefficient (known means)', function test( t ) {
	...
	for ( i = 0; i < N; i++ ) {
		...
		expected = pcorr2( d, means[ 0 ], means[ 1 ], true );
		if ( actual === expected ) {
			t.strictEqual( actual, expected, 'returns expected value. window: '+i+'.' );
		} else {
			delta = abs( actual - expected );
...
tape( 'if data has yet to be provided, the accumulator function returns `null` (known means)', function test( t ) {
	var acc = incrmpcorr2( 3, 3.0, 3.14 );
	t.strictEqual( acc(), null, 'returns expected value' );
	t.end();
});
...
tape( 'if only one datum has been provided and the means are known, the accumulator function may not return `0`', function test( t ) {
	var acc = incrmpcorr2( 3, 30.0, -100.0 );
	acc( 2.0, 1.0 );
...
tape( 'if the window size is `1` and the means are known, the accumulator function may not always return `0`', function test( t ) {
	...
	acc = incrmpcorr2( 1, 500.0, -500.0 ); // means are outside the range of simulated values so the correlation should never be zero
	for ( i = 0; i < 100; i++ ) {
		r = acc( randu()*100.0, randu()*100.0 );
...
tape( 'if provided `NaN`, the accumulated value is `NaN` for at least `W` invocations (unknown means)', function test( t ) {
	...
	for ( i = 0; i < data.length; i++ ) {
		...
		if ( isnan( expected[ i ] ) ) {
			...
			t.strictEqual( delta <= tol, true, 'window: '+i+'. expected: '+expected[ i ]+'. actual: '+v+'. tol: '+tol+'. delta: '+delta+'.' );
			t.strictEqual( acc(), v, 'returns expected value for window '+i );
		}
	}
